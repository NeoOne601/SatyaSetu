 ...
 * VERSION: 1.6.8
 * STATUS: STABLE (Native Reset Logic)
 */
...
class _UnlockScreenState extends State<UnlockScreen> {
  ...
  Future<void> _factoryReset() async {
    final confirmed = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(title: const Text("Factory Reset?"), content: const Text("Permanently wipe local identities and release filesystem locks? Proceed?"), actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text("Cancel")), ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: Colors.red), child: const Text("Reset"))]));
    
    if (confirmed == true) {
      final directory = await getApplicationSupportDirectory();
      // PRINCIPAL FIX: Call Native Rust Reset to ensure all handles are dropped before deletion
      final success = await widget.repo.resetVault(directory.path);
      
      if (success) {
        _pinController.clear();
        setState(() { _showReset = false; _isLoading = false; });
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vault Cleaned. You can now set a new PIN.")));
      }
    }
  }
