 ...
 * VERSION: 1.6.7
 * STATUS: STABLE (iMac Forensic Standard)
 */
...
class _UnlockScreenState extends State<UnlockScreen> {
  ...
  Future<void> _factoryReset() async {
    final confirmed = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(title: const Text("Factory Reset?"), content: const Text("This will permanently wipe all local identities and binding locks. Continue?"), actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text("Cancel")), ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: Colors.red), child: const Text("Purge Everything"))]));
    
    if (confirmed == true) {
      final directory = await getApplicationSupportDirectory();
      final vaultDir = Directory("${directory.path}/satya_vault");
      
      // PRINCIPAL FIX: Robust synchronous purge to ensure macOS filesystem synchronization
      try {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vault Cleaned. You can now set a fresh PIN.")));
      } catch (e) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Purge Failed: $e")));
      }
    }
  }
...
// ... HomeScreen & ScannerPage preserved from Phase 6.5 Baseline ...
