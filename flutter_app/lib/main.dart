 ...
 * VERSION: 1.5.1
 * STATUS: STABLE (UX Recovery)
 * DESCRIPTION:
 * Implements strict input clearing and focus management to prevent iMac 
 * keyboard conflicts.
 */
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        // Principal Fix: Fallback to system fonts to prevent network warnings
...
class _UnlockScreenState extends State<UnlockScreen> {
  ...
  void initState() {
    super.initState();
    // Principal Fix: Ensure the field is absolutely empty on launch
  ...
  Future<void> _attemptUnlock() async {
    ...
    try {
      ...
      if (success && mounted) {
        ...
        _pinController.clear();
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vault Access Denied: Incorrect PIN or Binding")));
      }
  ...
  Future<void> _confirmReset() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Binding mismatch suggests you are on a new device. Factory resetting will permanently wipe all local identities. Continue?"),
        actions: [
    ...
    if (confirmed == true) {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vault Initialized. Create a new identity.")));
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                // Principal Fix: Strict digit constraint and 6-digit length
              ...
              if (_showReset) TextButton(onPressed: _confirmReset, child: const Text("Hardware Mismatch? Reset Vault", style: TextStyle(color: Colors.redAccent))),
            ],
          ),
        ),
      ),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _createNew() async {
    ...
    final ok = await showDialog<bool>(context: context, builder: (context) => AlertDialog(title: const Text("New Identity"), content: TextField(controller: name), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Cancel")), ElevatedButton(onPressed: () => Navigator.pop(context, true), child: const Text("Generate"))]));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isSyncing ? const Center(child: CircularProgressIndicator()) : _identities.isEmpty ? Center(child: ElevatedButton(onPressed: _createNew, child: const Text("Create First Identity"))) : ListView.builder(padding: const EdgeInsets.all(16), itemCount: _identities.length, itemBuilder: (c, i) => Card(child: ListTile(leading: const Icon(LucideIcons.userCheck), title: Text(_identities[i].label), subtitle: Text(_identities[i].did, style: const TextStyle(fontSize: 10))))),
      floatingActionButton: FloatingActionButton(onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => ScannerPage(repo: widget.repo, identities: _identities))), child: const Icon(LucideIcons.scan)),
    );
  }
}
